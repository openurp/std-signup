...
object DocHelper {
  ...
  def toDoc(apply: SignupInfo): Array[Byte] = {
    ...
    data.put("disciplineCategory", apply.category.map(_.name).getOrElse("--"))
    data.put("squad", apply.squad.getOrElse(""))

    data.put("institution", apply.firstMajor.institution.name)
    data.put("firstMajor", apply.firstMajor.name)
    data.put("secondMajor", apply.secondMajor.map(_.name).getOrElse(""))

    import java.text.NumberFormat
    val nf = NumberFormat.getNumberInstance
    nf.setMinimumFractionDigits(2)
    nf.setMaximumFractionDigits(2)
    data.put("gpa", nf.format(apply.gpa.getOrElse(0d)))
